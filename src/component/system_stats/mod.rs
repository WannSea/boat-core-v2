    ...
    pub async fn collect_stats(metric_sender: MetricSender) {
        loop {
            ...
            if SETTINGS.get::<bool>("system.memory").unwrap() {
                match sys.memory() {
                    Ok(mem) => {
                        metric_sender.send(MetricMessage::now(Metric::MemUsedMb, (saturating_sub_bytes(mem.total, mem.free).as_u64() / 1024) as f32)).unwrap();
                        metric_sender.send(MetricMessage::now(Metric::MemTotal, (mem.total.as_u64() / 1024) as f32)).unwrap();
                    },
            ...
            if SETTINGS.get::<bool>("system.swap").unwrap() {
                match sys.swap() {
                    Ok(swap) => {
                        metric_sender.send(MetricMessage::now(Metric::SwapUsedMb, (saturating_sub_bytes(swap.total, swap.free).as_u64() / 1024) as f32)).unwrap();
                        metric_sender.send(MetricMessage::now(Metric::SwapTotal, (swap.total.as_u64() / 1024) as f32)).unwrap();
                    },
            ...
            if SETTINGS.get::<bool>("system.uptime").unwrap() {
                match sys.uptime() {
                    Ok(uptime) => {
                        metric_sender.send(MetricMessage::now(Metric::SystemUptime, uptime.as_secs_f32())).unwrap();
                    },
            ...
            if SETTINGS.get::<bool>("system.cpu_temp").unwrap() {
                match sys.cpu_temp() {
                    Ok(cpu_temp) => {
                        metric_sender.send(MetricMessage::now(Metric::CpuTemp, cpu_temp)).unwrap();
                    },
            ...
            if SETTINGS.get::<bool>("system.cpu_usage").unwrap() {
                match sys.cpu_load_aggregate() {
                    Ok(cpu)=> {
                        ...
                        metric_sender.send(MetricMessage::now(Metric::CpuUsageUser, cpu.user * 100.0)).unwrap();
                        metric_sender.send(MetricMessage::now(Metric::CpuUsageSystem, cpu.system * 100.0)).unwrap();
                    },
                    Err(x) => {
